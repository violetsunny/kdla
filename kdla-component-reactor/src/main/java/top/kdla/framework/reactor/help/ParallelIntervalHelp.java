...
public final class ParallelIntervalHelp {

    private final long interval;

    private final ConcurrentMap<String, Info> times = new ConcurrentHashMap<>();

    public static ParallelIntervalHelp create(Duration interval) {
        return new ParallelIntervalHelp(interval.toMillis());
    }
     ...
     * @return 间隔(毫秒)
     */
    public long next(@NotNull String key) {
     ...
     * @return 间隔(毫秒)
     */
    public long current(@NotNull String key) {
     ...
     * @return 延迟后的FLux
     */
    public <T> Flux<T> delay(@NotNull String key, @NotNull Flux<T> source) {
        return source
     ...
     * @return 延迟后的Mono
     */
    public <T> Mono<T> delay(@NotNull String key, @NotNull Mono<T> source) {
     ...
     * @return 延迟Mono
     */
    public Mono<Void> delay(@NotNull String key) {
        return this
     ...
     * @return 转换后的类型
     */
    public <S> S delay(@NotNull String key, S source, BiFunction<S, Duration, S> mapper) {
        long delay = next(key);
        if (delay > 0) {
